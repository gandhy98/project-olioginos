...
<!DOCTYPE html>
	...
	<input type="hidden" name="txtfiltro_buscar" id="txtfiltro_buscar">	
		...
		<input type="hidden" id="txtnom_tabla" value='AREAS_TRABAJO'>	
	    	...
	    	<div class="col-md-4" style="text-align:left;">
	          ...
	          <div class="dropdown">	            				
				...
				<div class="col-md-4" style="align-items: center;" >
				   ...
				   <input type="text" class="form-control" id="txtbuscar_areas" placeholder="Buscar "oninput="convertirAMayusculas(this)">
...
<div class="modal" id="exampleModal">
    ...
    <div class="modal-content">
      	...
      	<div class="modal-header">
		    <h5 class="modal-title text-primary" id="exampleModalLabel">Registrar Areas de Trabajo</h5>		    
		    </div>

		    	<input type="hidden" class="form-control" id="txtid_area">
					...
					<div class="row g-3">
						...
						<div class="col-md-12">
					  	<div class="form-floating">
					    	<input type="text" class="form-control" id="txtarea_trabajo" placeholder="Área de trabajo" oninput="convertirAMayusculas(this)">
  							<label for="txtarea_trabajo"><b>Área de trabajo</b></label>
					  	</div>
					  	<div id="div_listar_areas_registradas"></div>
						</div>

						<div class="col-md-12">
					  	<div class="form-floating">
					    	<input type="text" class="form-control" id="txtarea_encargado" placeholder="Cargo" oninput="convertirAMayusculas(this)">
  							<label for="txtarea_encargado"><b>Cargo</b></label>
					  	</div>
						</div>

						<div class="col-md-12" style="text-align: left; padding: 15px;">		
							<div class="form-check form-switch">
							  <input class="form-check-input" type="checkbox" role="switch" id="txttipo_asignacion" value="NO">
							  <label class="form-check-label" for="txttipo_asignacion">Asignar equipo</label>
							</div>
             </div>
		    ...
		    <div class="modal-footer">
		    		<button type="button" class="btn btn-outline-success btn-sm"  id="btn_agregar_area" data-bs-dismiss="modal"><i class="fas fa-save"></i> Grabar</button>
...
$(document).on("click","#btn_seleccionar_area",function(){

	$("#txtarea_trabajo").val($(this).text());
	$("#div_listar_areas_registradas").fadeOut();

})

$('#txtarea_trabajo').keyup(function(){

  var axbuscar = $("#txtarea_trabajo").val();  

  if (axbuscar != '') {

    $.ajax({
      url:"Areas_trabajo_funciones.php",
      method: "POST",
      data: {param:4,txtarea_trabajo:axbuscar},
      success : function(data){

        $('#div_listar_areas_registradas').fadeIn();
        $('#div_listar_areas_registradas').html(data);
...
$(document).on("click","#btn_areas_todos",function(){
...
$(document).on("click","#btn_ordenar_cargo_asc",function(){
		$("#txttipoorden").val($(this).data('tipoorden'))		
...
$(document).on("click","#btn_ordenar_cargo_desc",function(){
...
$(document).on("click","#btn_ordenar_area_desc",function(){
	...
	$(document).on("keyup","#txtbuscar_areas",function(){
		listar()
	})

$(document).on("click","#btn_eliminar_area",function(){
	
	$("#txtid_area").val($(this).data('id'))	
	var axid_area = $("#txtid_area").val();
...
Swal.fire({
  ...
  if (result.isConfirmed) {
      ...
      url:"Areas_trabajo_funciones.php",
      method: "POST",
      data: {param:3,txtid_area:axid_area,
			txtmodulo:axmodulo,
...
$(document).on("click","#btn_editar_area",function(){
	...
	var axpermiso_editar = $("#txtpermiso_editar").val()	
	 ...
	 $.ajax({
      ...
      url:"Areas_trabajo_funciones.php",
      method: "POST",
      data: {param:2,txtid_area:axid_area},
      success : function(Traedatos){
      	...
      	var json = JSON.parse(Traedatos);
			...
			if (json.status == 200){				
				...
				$("#txtid_area").val(json.ID_AREA);
				$("#txtarea_trabajo").val(json.AREA_TRABAJO);
				$("#txtarea_encargado").val(json.AREA_ENCARGADO);
				$("#txtid_empresa").val(json.ID_EMPRESA)
				$('#txttipo_asignacion').val(json.ASIG_EQUIPOS);

				if(json.ASIG_EQUIPOS=='SI'){
					 $('#txttipo_asignacion').prop('checked', true);
				}else{
					 $('#txttipo_asignacion').prop('checked', false);
				}
...
$(document).on("click","#btn_nuevo",function(){
	$("#txtparametros").val(0)
	$("#txtarea_trabajo").val('');
$("#txtarea_encargado").val('');
})


$(document).on("click","#btn_agregar_area",function(){

var axid_empresa = $("#txtid_empresa").val()
var axid_area = $("#txtid_area").val();
var axarea_trabajo = $("#txtarea_trabajo").val();
var axarea_encargado = $("#txtarea_encargado").val();
...
var axid_usuario = $("#txtid_usuario").val()
 ...
 $.ajax({
      ...
      data: {param:1,
      	...
      	txtid_area:axid_area,
				txtarea_trabajo:axarea_trabajo,
				txtarea_encargado:axarea_encargado,				
...
function listar() {
	...
	var axidempresa = $("#txtid_empresa").val();
		var axbuscar = $("#txtbuscar_areas").val();	
...
function traer_nom_menu(){
	...
	$.ajax({
		...
		success : function(data){
			...
			$("#titulo_formulario").html("<i class='bi bi-diagram-3-fill' style='font-size:25px;'></i> "+data +" <button type='button' id='btn_nuevo'  class='btn btn-outline-primary btn-sm' data-bs-toggle='modal' data-bs-target='#exampleModal'><i class='bi bi-file-earmark-plus'></i> Nuevo</button> <a href='Proyectos.php' id='btn_retornar_de_detalles'  class='btn btn-outline-danger btn-sm' hidden><i class='bi bi-arrow-return-left'></i> Retornar</a>")		
...
function Verifica_permiso_editar(){
	...
	var axpermiso_1 = 'AREAS DE TRABAJO EDITAR';
...
function Verifica_permiso(){
	...
	var axiduser =$("#txtid_usuario").val();
	var axtipo_beneficiario =$("#txttipo_beneficiario").val();	
	var axpermiso_1 = 'AREAS DE TRABAJO';	

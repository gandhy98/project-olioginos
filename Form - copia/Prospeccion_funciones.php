...
case '1':
   ...
   if($axparametros==0){
      ...
      $sqlgrabar = "UPDATE MK_PROSPECTOS_CZ SET CODIGO_PST_CZ='$axcodigo_pst_cz',DESCRIPCION_PST='$axdescripcion_pst',ID_PROYECTO='$axid_proyecto',ID_MEDIO_CAPTACION='$axid_medio_captacion',ID_CANAL='$axid_canal',NUM_CELULAR_PST='$axnum_celular_pst',ID_EMPRESA='$axid_empresa' WHERE ID_PROSPECTO_CZ='$axid_prospecto_cz'";
  ...
  // echo $sqlgrabar;
   ...
   if($rsgrabar){
    ...
    if($axtipo_asignacion=='AUTO'){
        ...
        $axid_prospecto_cz = get_row('MK_PROSPECTOS_CZ','ID_PROSPECTO_CZ','CODIGO_PST_CZ',$axcodigo_pst_cz);
        ...
        $sqlgrabar_dt = "INSERT INTO MK_PROSPECTOS_DT (ID_PROSPECTO_CZ,FECHA_ASIGNACION,HORA_ASIGNACION,ID_USUARIO,ID_DESVIACION) VALUES ('$axid_prospecto_cz','$axfecha_reg_prospecto','$axhora_reg_prospecto','$axid_usuario_vendedor','$axid_desviacion')";
        $rsgrabar_dt = odbc_exec($con,$sqlgrabar_dt);

        $sqlactualizar = "UPDATE MK_PROSPECTOS_CZ SET NOM_EJECUTIVO='$axnom_ejecutivo',ID_EJECUTIVO='$axid_usuario_vendedor',FECHA_ASIGNACION_ACTUAL='$axfecha_reg_prospecto',HORA_ASIGNACION_ACTUAL='$axhora_reg_prospecto' WHERE ID_PROSPECTO_CZ='$axid_prospecto_cz'";
...
case '2':
	...
	$sql6 = "SELECT * FROM MK_PROSPECTOS_CZ WHERE ID_PROSPECTO_CZ = '$axid_prospecto_cz'";
...
if($axtipoorden==""){
    ...
    if($axtipo_busqueda=='FILTRAR'){
        ...
        $sql6 ="SELECT TOP 20 *,COUNT(*) OVER () AS cant_registros FROM $axnom_tabla WHERE ID_EMPRESA='$axidempresa' ORDER BY ID_PROSPECTO_CZ ASC";  
        $axcontador = 20 .' de '.'<a href="#" id="btn_prospectos_todos" data-tipoorden="TODOS" style="text-decoration:none; color:black;" title="Click para visualizar todos...">'.$axtotal_registros.'</a>';
            ...
            $sql6 ="SELECT TOP $axtotal_registros *,COUNT(*) OVER () AS cant_registros FROM $axnom_tabla WHERE ID_EMPRESA='$axidempresa' ORDER BY ID_PROSPECTO_CZ ASC";
    ...
    $sql6 ="SELECT *, COUNT(*) OVER () AS cant_registros FROM $axnom_tabla WHERE ID_EMPRESA='$axidempresa' AND USUARIO+' '+NOM_USUARIO+' '+COD_USUARIO+' '+AREA_TRABAJO+' '+AREA_CARGO like '%".$axbuscaregistro."%' ORDER BY ID_PROSPECTO_CZ ASC";
  }
...
echo "<table class='table table-hover table-sm'>
    ...
    <tr class='table-secondary'>
        ...
        <tr>
            ...
            <th style='text-align: left;'>ESTADO ASIGNACION</th>            
            <!--th style='text-align: center;'>ASIGNADO</th>            
            <th style='text-align: center;'>SEGUIMIENTO</th-->            
            <th style='text-align: center;'>TIEMPO RESPUESTA</th>            
...
if (odbc_num_rows($RSListar) > 0) {
    ...
    while ($fila = odbc_fetch_array($RSListar)) {
        ...
        $estado = $fila['ESTADO'];
        $idprospecto=$fila['ID_PROSPECTO_CZ'];
        ...
        $cardClass = ''; // Clase adicional para el color de la sombra
        $axcant = get_row('MK_PROSPECTO_CANTIDAD','ID_PROSPECTO_CZ','ID_PROSPECTO_CZ',$axId_prospecto_cz);

        if($axcant > 0){
            $actividad = '<span class="badge text-bg-danger">'.$axcant.'</span>';
        }else{
            $axcant=0;
            $actividad = '<span class="badge text-bg-danger">'.$axcant.'</span>';
        }
        ...
        if ($axEstado == 'ASIGNADO' || $axEstado == 'REASIGNADO' ) {
            ...
            $axboton_1 ="<a href='#' class='$axclass_color' title='Click para desviar el prospecto' id='btn_desviar_prospecto' data-coordinador='$axnom_coordinador' data-id_ejecutivo='$axid_ejecutivo' data-id_coordinador='$axid_coordinador' data-nom_ejecutivo='$axnom_ejecutivo' data-bs-toggle='modal' data-bs-target='#mdl_reasignar_prospectos_dt' data-motivo='$axmotivo' data-id='$axId_prospecto_cz' data-titulo='$axtitulo'  style='text-decoration:none;'> ASIGNADO A <b>$axejecutivo_asignado  <i class='bi bi-dash-circle-fill'></i> </b><br> COORDINADOR : <b> $axuser_coordinador </b></a>";
            ...
            $axboton_1 ="<a href='#' class='$axclass_color' title='Click para asignar el prospecto' id='btn_asignar_prospecto' data-id_coordinador='$axid_coordinador' data-bs-toggle='modal' data-bs-target='#mdl_asignar_prospectos_dt' data-id='$axId_prospecto_cz' data-titulo='$axtitulo'  style='text-decoration:none;'><b>NO ASIGNADO <i class='bi bi-plus-circle-fill' ></i></b> <br> COORDINADOR : <b> $axuser_coordinador </b></a>";
         ...
         echo " <tr>
                <td style='text-align:center;'>$axId_prospecto_cz </td>
                ...
                <div class='btn-group dropstart'>
                    ...
                    <a href='#' class='dropdown-item text-info' data-bs-toggle='modal' data-bs-target='#mdl_registrar_prospectos' title='Click para editar el prospecto' id='btn_editar_prospecto_cz' data-id='$axId_prospecto_cz' style='text-decoration:none;'><i class='bi bi-pencil' style='font-size:20pX;'></i> Editar</a>  
                    <!--a href='#' class='dropdown-item text-danger ' title='Click para eliminar el prospecto' id='btn_eliminar_prospecto_cz' data-id='$axId_prospecto_cz' style='text-decoration:none;'><i class='bi bi-trash3-fill' style='font-size:20pX;'></i> Eliminar </a-->
                    <div><hr class='dropdown-divider'></div>
                    </ul>
                </div>
...
function calcularTiempoEIndicadorxxx($estado, $fechaAsignacion, $horaAsignacion, $fechaSeguimiento, $horaSeguimiento,$idprospecto) {
    date_default_timezone_set("America/Lima");
    global $con;
    ...
    // Lógica para determinar el indicador según el tiempo
        if ($minutosTotales <= 30) {
            ...
            $indicador='1';
            $sqlactualizar = "UPDATE MK_PROSPECTOS_CZ SET TIEMPO='$minutosTotales_guardar', INDICADOR='$indicador' WHERE ID_PROSPECTO_CZ='$idprospecto'";
            $rsactualizar = odbc_exec($con, $sqlactualizar);
            ...
            $dias = '1 dia a mas';
            //$dias = floor($minutosTotales / 1440); // Obtener los días completos
            //$horasRestantes = $minutosTotales % 1440 / 60; // Obtener las horas restantes
    ...
    //    // Otro caso (puedes ajustar según tus requisitos)
        ...
        $sqlactualizar = "UPDATE MK_PROSPECTOS_CZ SET TIEMPO='N/A', INDICADOR='6' WHERE ID_PROSPECTO_CZ='$idprospecto'";
        $rsactualizar = odbc_exec($con,$sqlactualizar);

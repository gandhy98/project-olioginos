	      	...
	      	<input type="hidden" class="form-control" id="txtid_producto">
				...
				<div class="row g-3" id="div_cabecera_formulario" hidden>
					...
					<div class="modal-footer">
					   	...
					   	<button type="button" class="btn btn-outline-danger btn-sm" id='btn_cerrar_formulario' ><i class="fas fa-door-closed" ></i> Cerrar</button>
					</div>
				...
				<div class="row g-3" id="div_cabecera_listado" hidden>
					...
					<div class="input-group mb-3">
					  ...
					  <button type="button" class="btn btn-outline-success btn-sm"  id="btn_nuevo_gasto"><i class="bi bi-plus-circle-fill"></i> Nuevo gasto</button>
					</div>
					</div>
...
$(document).on("click","#btn_eliminar_rendicion",function(){
 ...
 var axestado_rendicion = $("#txtestado_rendicion").val()
 	...
 	if(axestado_rendicion=='RENDIDA'){
		Swal.fire('Aviso!','La rendición no puede ser ELIMINADA, puesto que tiene estado RENDIDA','warningr')
	}else{

		Swal.fire({
			  title: "Estas seguro?",
			  text: "Se eliminara la rendición junto con su detalle de gastos!",
			  icon: "warning",
			  ...
			  if (result.isConfirmed) {
			    	...
			    	$.ajax({
				      ...
				      method: "POST",
				      data: {param:10,txtid_rendicion_cz:axid_rendicion_cz},
				      success : function(data){
				      		...
				      		/*	
				      			...
				      			Swal.fire("Aviso", "Error al intentar eliminar el detalle de la rendición...", "error");						
				      		}else if(data==2){
				      			Swal.fire("Aviso", "Error al intentar eliminar la cabecezera de la rendición...", "error");						
				      		}
				      		*/
...
$(document).on("click","#btn_eliminar_gasto_rendicion",function(){

 $("#txtid_rendicion_cz").val($(this).data('id'))
 $("#txtid_rendicion_dt").val($(this).data('iddt'))
	 ...
	 $.ajax({
	      ...
	      success : function(data){
	      		...
	      		if(data==0){
	      			...
	      			Swal.fire("Aviso", "Error al intentar eliminar el registro...", "error");						
	      		}
...
$(document).on("click","#btn_listar_productos_egresos",function(event){

	$("#txtnom_producto").val($(this).data('nom_pro'));
	$("#txtid_producto").val($(this).data('id'));
	console.log($("#txtid_producto").val())
	$("#listar_productos_servicios").fadeOut();
})
...
$("#txtnom_producto").keyup(function(){
	...
	/**por aqui tomamos y evimos y pasamos y recibimos los datos con ajax */
	var axbuscar_dato = $("#txtnom_producto").val();
	  ...
	  if (axbuscar_dato != '') {
	    ...
	    $.ajax({
	      url:"egresos_rendicion_funciones.php",
	      method: "POST",
	      data: {param:7,txtnom_producto:axbuscar_dato},
	      success : function(data){
...
$(document).on("keyup","#txtbuscar_gasto",function(event){
	traer_detalle_rendicion()
})
...
function limpiar(){
	...
	$("#txtn_serie").val('')
	$("#txtdocumento").val('')
	$("#txtmonto_gasto").val(0)

	$("#txtnom_proveedor").val('')
	$("#txtnom_producto").val('')
	$("#txtid_td").val('')
...
$(document).on("keyup","#txtbuscar",function(event){
	listar_egresos()
})

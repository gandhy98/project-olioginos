...
case '0': // listar usuarios
	...
	$axtipo_cotizacion = $_POST['txttipo_cotizacion']; 	
	$axanoactual = $_POST['txtanoactual']; 

	if($axanoactual==''){
		$axanoactual = date('Y',strtotime($axfecharegistro));
	}
	...
	if($axbuscar==""){
		...
		if($axfiltro_busca_fecha=='FECHA'){
			$sql6 ="SELECT TOP 250 * FROM COTIZACION_REPORTE_CZ WHERE ID_LOCAL='$axidlocal' AND FECHA_COTIZACION ='$axfecharegistro' AND TIPO_COTIZACION='$axtipo_cotizacion' ORDER BY NUM_COTIZACION DESC";		
		}else{
			$sql6 ="SELECT  TOP 250 * FROM COTIZACION_REPORTE_CZ WHERE ID_LOCAL='$axidlocal'  AND TIPO_COTIZACION='$axtipo_cotizacion' AND ANO_COTIZACION='$axanoactual' ORDER BY NUM_COTIZACION DESC";		
		}
	
	}else{

		$sql6 ="SELECT TOP 250 * FROM COTIZACION_REPORTE_CZ WHERE TIPO_COTIZACION='$axtipo_cotizacion' AND ANO_COTIZACION='$axanoactual' AND ID_LOCAL='$axidlocal' AND RAZON_SOCIAL+NUM_COTIZACION LIKE  '%".$axbuscar."%' ORDER BY NUM_COTIZACION DESC";	
	...
	<table class='table table-sm table-striped table-hover table-bordered table-sm'>
		...
		<tr>
			...
			<th scope='col' style='text-align: left;'>Elaborado por</th>
			<th scope='col' style='text-align: center;'>Fecha</th>			
			<th scope='col' style='text-align: center;'>Num. Cotización</th>			
	...
	if ($result6){
 	while ($row=odbc_fetch_array($result6)){ 
 		...
 		$it = $it+1;
 		$axnum_proforma_1 =$row['NUM_COTIZACION'];

 		$axtipo_cotizacion=$row["TIPO_COTIZACION"]; 
 	
 		$id = $row["COD_MOV_PRF"];
 		$axestadoelectro=$row["ESTADO_ATENDIDO"]; 
 		$axelaboradopor =$row["USUARIO"];
		$id_producto=$row["ID_COTIZACION_CZ"];
		$axestado_cotizacion = $row["ESTADO_COTIZACION"];

		$axcoti_mostrar ='CT-'.$row['NUM_COTIZACION'].'-'.$row['ANO_COTIZACION'];
 	...
 	echo "
 		<tr>
 			<td style='text-align: center; width:15px;'>".$it."</td>  	
 			<td style='text-align: left; width:15px;'>".$axelaboradopor."</td>  			
 			<td style='text-align: center; '>".date("d-m-Y",strtotime($row["FECHA_COTIZACION"]))."</td>  				
 			<td style='text-align: center;'>".$axcoti_mostrar."</td>
 	 		<td style='text-align: left;'>".$row["RAZON_SOCIAL"]."</td>
			<td style='text-align: left;'>".$row["ESTADO_COTIZACION"]."</td>
			...
			<td style='text-align: center; '>			
				...
				<a href='#' class='dropdown-item text-info' id='btn_editar_cotizacion' data-id='$id_producto' data-bs-toggle='modal' data-bs-target='#exampleModal'><b><i class='bi bi-pencil' ></i> Editar</a></b>
				<a href='#' class='dropdown-item text-danger' id='btn_eliminar_cotizacion' data-estado='$axestado_cotizacion' data-id='$id_producto' ><b><i class='bi bi-trash3-fill'></i> Eliminar</a></b>
				<a href='#' onclick='imprimir_cotizacion($id_producto)' class='dropdown-item text-success' id='btn_imprimir_producto' data-id='$id_producto' data-bs-toggle='modal' data-bs-target='#exampleModal'><b><i class='bi bi-printer' ></i> Imprimir</a></b>
				<div><hr class='dropdown-divider'></div>				  	
...
case '2':
	...
	if ($result6){
 	...
 	while ($row=odbc_fetch_array($result6)){ 
 		...
 		$id = $row["ID_PRODUCTO"];
 		$prs_venta=$row["PRECIO_VENTA_CIGV"];
 		$prs_compra=$row["PRECIO_COMPRA_CIGV"];
...
case '10':
	...
	$axtipo_cotizacion = $_POST['txttipo_cotizacion'];
	$axano_actual = date('Y',strtotime($axfechaactual));

	$axid_cotizacion_cz = get_row('COTIZACION_CZ','ID_COTIZACION_CZ','COD_MOV_PRF',$axcodproformacz);
	
	
	$SQLInsertar = "INSERT INTO COTIZACION_DT (ID_COTIZACION_CZ,COD_MOV_PRF,ID_PRODUCTO,CANT_SALIDA,PRECIO_V,PRECIO_C,DSCTOS_SALIDA,VALOR_SALIDA,IGV_SALIDA,GRAVADAS_SALIDA,INAFECTO_SALIDA,EXONERADO_SALIDA,TOTAL_SALIDA) VALUES ('$axid_cotizacion_cz','$axcodproformacz','$axidinsumo','$axcantidad','$axprecio_venta','$axprecio_compra','$axdscto','$axvalor_venta','$axigv_venta','$axgravada','$axinafecta','$axexonerada','$axtotalventa')";

	$RSInsertar=odbc_exec($con,$SQLInsertar); 

	if($RSInsertar){

	$sql6 = "SELECT * FROM LISTA_COTIZACION_DT WHERE COD_MOV_PRF = '".$axcodproformacz."' ORDER BY NOM_PRODUCTO";
	//echo $sql6;
	echo "
	<table class='table table-sm table-hover'>";
		...
		$SQLTotal = "SELECT SUM(TOTAL_SALIDA) AS TT FROM LISTA_COTIZACION_DT WHERE COD_MOV_PRF = '".$axcodproformacz."'";
...
case '11':
	...
	$sqleliminar = "DELETE FROM COTIZACION_DT WHERE COD_MOV_PRF ='$axcodmov' AND ID_PRODUCTO='$axidinsumo'";
...
case '12':
	...
	$axcodproformacz = trim($_POST['txtcodmovprof']);
	$sql6 = "SELECT * FROM LISTA_COTIZACION_DT WHERE COD_MOV_PRF = '".$axcodproformacz."' ORDER BY ID_COTIZACION_DT";
	//echo "$sql6";
	echo "
...
case '13':
	...
	$axfechaemision = $_POST['txtfecha_emision'];
	$axidlocal = $_POST['txtidlocal'];	
	$axmoneda = $_POST['txtmoneda'];
	$axtipodoc = $_POST['txttipodoc'];
	...
	$SQLActualizar = "UPDATE COTIZACION_CZ SET ID_BENEFICIARIO='$axidbeneficiario',ID_USUARIO='$axcodusuario',FECHA_COTIZACION='$axfechaemision',ID_LOCAL='$axidlocal',MONEDA='$axmoneda',ID_DT='$axtipodoc',MEDIO_PAGO='$axmediopago',FORMA_PAGO='$axformapago',ESTADO_FORMA_PAGO='$axestadoformapago',ESTADO_COTIZACION='PENDIENTE',NOM_CLIENTE_PROF='$axnom_cliente_pedido',NUM_COTIZACION='$axnum_proforma_1',ESTADO_TRANSFORMAR='PENDIENTE' WHERE COD_MOV_PRF='$axcodproformacz'";

	$RSActualizar = odbc_exec($con,$SQLActualizar);

//NUM_COTIZACION,FECHA_COTIZACION,ID_BENEFICIARIO,ID_LOCAL,ESTADO_COTIZACION,ID_USUARIO,FORMA_PAGO,,MONEDA,COD_MOV_PRF,ID_DT,MEDIO_PAGO,ESTADO_FORMA_PAGO,,NOM_CLIENTE_PROF,
...
$axid_cotizacion = $_POST['axid_cotizacion']; 
$axidlocal = $_POST['txtidlocal']; 

$SQLDelete_dt = "DELETE FROM COTIZACION_DT WHERE ID_COTIZACION_CZ='$axid_cotizacion'";
$RSDelete_dt = odbc_exec($con, $SQLDelete_dt);

//echo $SQLDelete;

if($RSDelete_dt){

	$SQLDelete_cz = "DELETE FROM COTIZACION_CZ WHERE ID_COTIZACION_CZ='$axid_cotizacion'";
...
case '32':
	...
	$SQLTipodocumentos = "SELECT * FROM COTIZACION_REPORTE_CZ WHERE COD_MOV_PRF ='$axcodproformacz' ORDER BY COD_MOV_PRF" ;
	$result1=odbc_exec($con,$SQLTipodocumentos);
...
case '34':
	...
	$SQLActualizar = "UPDATE PROFORMA SET TIPO_PROFORMA='COTIZACION' WHERE COD_MOV_PRF='$axcodproformacz' AND ID_LOCAL='$axidlocal'";
	$RSActualizar = odbc_exec($con,$SQLActualizar);
	//echo $SQLActualizar;
...
case '36':
	...
	$sql ="INSERT INTO COTIZACION_CZ (ID_LOCAL,TIPO_COTIZACION,ID_USUARIO,COD_MOV_PRF,ANO_COTIZACION)VALUES('$axidlocal','$axtipo_cotizacion','$axcodusuario','$axcodmovprof','$axanoactual')";
...
function contarRegistros($local,$tipo) {
    global $con;
    $querysql = "SELECT  TOP 1 NUM_COTIZACION FROM COTIZACION_CZ WHERE ID_LOCAL='$local' AND TIPO_COTIZACION='$tipo' order by NUM_COTIZACION desc";
    //echo $querysql.'<br>';
    $query = odbc_exec($con, $querysql);
    $rw = odbc_fetch_array($query);
    $value = $rw['NUM_COTIZACION'];
    return $value;
}

...
$axtipo_beneficiario = $_POST['txttipo_beneficiario']; 	
	...
	if($axbuscaregistro==""){
		...
		if($axid_vendedor=='Seleccionar'){
			...
			$SQLBuscar = "SELECT TOP 50 * FROM BENEFICIARIOS WHERE ID_EMPRESA = '$axid_empresa' AND TIPO_PROV_CLIE='$axtipo_beneficiario' AND ID_VENDEDOR='$axid_vendedor' order by COD_INTERNO ASC";
		}
				...
				if($axid_vendedor=='Seleccionar'){
					...
					$SQLBuscar ="SELECT TOP 50 *  FROM BENEFICIARIOS WHERE ID_EMPRESA = '$axid_empresa' AND TIPO_PROV_CLIE='$axtipo_beneficiario'  AND ID_VENDEDOR='$axid_vendedor' AND RAZON_SOCIAL like '%".$axbuscaregistro."%' ";
				}
	...
	echo "
		...
		<tr>
			...
			<th class='ocultar' style='text-align: left; width:8%;'>No documento</th>
			<th style='text-align: left;'>Razón social</th>
			<th class='ocultar' style='text-align: left;'>Direc. Entrega</th>
 	...
 	while ($fila=odbc_fetch_array($RSBuscar)){ 
		...
		if ($domic_entrega_pred==''){
			if($axtipo_beneficiario=="CLIENTE"){
				...
				$domic_entrega_pred =$fila['DIRECCION_FISCAL'];			
			}
			}
		...
		$axtipodoc = get_row('TB2_DOCIDENTIDAD','ABREV_TD','ID_DOC',$id_doc);

 	echo "
 		<tr> 		
 			<td class='ocultar' style='text-align: center;'>".$it."</td> 
 			<td class='ocultar' style='text-align: left; width:8%;'>".$axtipodoc."</td> 
 			<td class='ocultar' style='text-align: left; width:8%;'>".$axruc."</td> 
...
$axcod_interno = trim($_POST['txtcod_interno']); 	
$axcod_interno_traer = trim($_POST['txtcod_interno']); 	
...
$axid_vendedor = $_POST['txtid_vendedor']; 	
if($axtipo_beneficiario=="PROVEEDOR"){
	$axid_vendedor=0;
}
$txtcta_pagos = $_POST['txtcta_pagos']; 	
$txtbanco_pagos = $_POST['txtbanco_pagos']; 	
$txtcta_detraccion = $_POST['txtcta_detraccion']; 	
...
if($axparametros==1){
	...
	if($RSInsert){
		...
		$axcod_interno_grabar =$axcod_interno;
	
		$SQLGrabar = "INSERT INTO BENEFICIARIOS (TIPO_PROV_CLIE,ID_DOC,RUC_BENEF,COD_INTERNO,NOM_COMERCIAL,RAZON_SOCIAL,PATERNO,MATERNO,NOMBRES,DIRECCION_FISCAL,DIRECCION_ENTREGA,TXT_URB_EMIS,DISTRITO,REFERENCIA,TELEFONO,EMAIL_PROVEEDOR,ESTADO,ID_EMPRESA,ID_VENDEDOR,ID_DIRECCION,CUENTA_PAGOS,BANCO_PAGOS,CUENTA_DETRACCION,DISTRITO_ENTREGA,REFERENCIA_ENTREGA) VALUES ('$axtipo_beneficiario','$axid_doc','$axruc_benef','$axcod_interno_grabar','$axnombre_comercial','$axrazon_social_cliente','$axpaterno','$axmaterno','$axnombre','$axdomicilio_fiscal','$axdomicilio_entrega','$axurb_emis','$axdistrito','$axreferencia','$axtelefono','$axemail_cliente','$axestado_cliente','$axid_empresa','$axid_vendedor','$axid_direccion','$txtcta_pagos','$txtbanco_pagos','$txtcta_detraccion','$axdistrito_alter','$axreferencia_1')";
		
	
}else{

	
$SQLGrabar ="UPDATE BENEFICIARIOS SET TIPO_PROV_CLIE='$axtipo_beneficiario',ID_DOC='$axid_doc',RUC_BENEF='$axruc_benef',COD_INTERNO='$axcod_interno',NOM_COMERCIAL='$axnombre_comercial',RAZON_SOCIAL='$axrazon_social_cliente',PATERNO='$axpaterno',MATERNO='$axmaterno',NOMBRES='$axnombre',DIRECCION_FISCAL='$axdomicilio_fiscal',DIRECCION_ENTREGA='$axdomicilio_entrega',TXT_URB_EMIS='$axurb_emis',DISTRITO='$axdistrito',REFERENCIA='$axreferencia',TELEFONO='$axtelefono',EMAIL_PROVEEDOR='$axemail_cliente',ESTADO='$axestado_cliente',ID_EMPRESA='$axid_empresa',ID_VENDEDOR='$axid_vendedor',CUENTA_PAGOS='$txtcta_pagos',BANCO_PAGOS='$txtbanco_pagos',CUENTA_DETRACCION='$txtcta_detraccion',DISTRITO_ENTREGA='$axdistrito_alter',REFERENCIA_ENTREGA='$axreferencia_1' WHERE ID_BENEFICIARIO='$axid_beneficiario'";
...
if($axid_vendedor=="Seleccionar"){
	if($axtipo_beneficiario=='CLIENTE'){
		$respuesta2=1;
		//echo "No selecciono el vendedor";
		//echo 'llega hasta aqui 4';
	}else{

		$RSGrabar = odbc_exec($con,$SQLGrabar);	
		//echo 'llega hasta aqui 5';
	}
	
}else{
	$RSGrabar = odbc_exec($con,$SQLGrabar);
	//echo 'llega hasta aqui 6';
}
...
/*
	...
	*/
	...
	$respuesta =0;
	echo $respuesta.$respuesta2.$SQLGrabar;
...
$axcod_interno_traer = trim($_POST['txtcod_interno_traer']); 	
...
$axcod_interno = generateRandomCode();

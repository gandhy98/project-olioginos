	            ...
	            <div class="card">
	              <div class="card-header">
	               <h5 class="text-danger">
	               		<img src="../icon/pedidos_gestion.png" style="width: 50px; height: 50px;"> ORDEN DE COMPRA 
		      ...
		      <div class="card-body border border-danger">	
		      	...
		      	<div class="form-group col-md-7">
				    ...
				    <input type="hidden" class="form-control" id="txtnom_cliente_pedido" >
						<input type="text" class="form-control" id="txtnum_proforma_1" value='0'  placeholder="Número Cotización" style="text-align:center;">
						</div>
				...
				<div class="row">
					...
					<a href="ORDEN_COMPRA.php" class="btn btn-outline-dark" ><img src='../icon/cancelar2.png'> Cancelar</a>					
			  	</div>
			  </div>	
...
function Generar_CodPorforma(){
	...
	var axtipo_ORDEN_COMPRA = $("#txttipo_ORDEN_COMPRA").val();
	...
	data: {	param:35,
			...
			txttipo_ORDEN_COMPRA:axtipo_ORDEN_COMPRA
			
		},
		success : function(data){
			$("#txtnum_proforma_1").val(data)
			$("#txtnum_proforma").html('Detalle de Cotización | '+data)
		}
})
...
function listarStock(){
	...
	$.ajax({
		...
		url:"ORDEN_COMPRA_funciones.php",
		method: "POST",
		data: {param:2,txtidempresa:axidempresa,txtbuscararticulo:axbuscar,txtidlocal:axidlocal,txttipo_ORDEN_COMPRA:axtipo_ORDEN_COMPRA},
			success : function(listar){
		...
		data: {param:9,txtidlocal:axidlocal,txtidinsumo:axidinsumo},
			...
			var json = JSON.parse(traer_insumos);
				...
				if (json.status == 200){
					...
					//alert(axcantidad+'|'+axstock_actual);
					//alert(axprecio_venta);

					//if ( parseInt(axcantidad) < parseInt(axstock_actual) ){
					if ( parseInt(axcantidad) == parseInt(axstock_actual) ){
...
$(document).on("click","#btvender",function(){
	...
	var axfechaemision = $("#txtfecharegistro").val();
	var axidlocal = $("#txtidlocal").val();
	var axmoneda = $("#txtmoneda").val();
	var axtipodoc = $("#txttipodoc").val();
		...
		$.ajax({
	      ...
	      data: {param:13,
		  	...
		  	txtidcliente:axidcliente,
				...
				txtfecharegistro:axfechaemision,
				txtidlocal:axidlocal,
				txtmoneda:axmoneda,
	      	...
	      	success : function(grabar_Cabecera){
	      		...
	      		if(grabar_Cabecera==0){
	      			...
	      			reporte_ORDEN_COMPRA();
	      			//window.open("ORDEN_COMPRA.php",'_self');	
...
$(document).on("click","#btn_editar_producto",function(){
	...
	asql="select * from ORDEN_COMPRA_reporte_cz where ID_ORDEN_COMPRA_CZ="+$(this).data("id");
	regresamatrizjsonasync(asql,'aORDEN_COMPRA');
	console.log(aORDEN_COMPRA);
	asql="select * from lista_ORDEN_COMPRA_dt where cod_mov_prf='"+aORDEN_COMPRA[0].COD_MOV_PRF+"'";
	regresamatrizjsonasync(asql,'adetORDEN_COMPRA');
	$("#txtcodmovprof").val(aORDEN_COMPRA[0].COD_MOV_PRF);
	axcodproformacz=aORDEN_COMPRA[0].COD_MOV_PRF;
	console.log(adetORDEN_COMPRA);
	...
	$.ajax({
      url:"ORDEN_COMPRA_funciones.php",
      method: "POST",
      data: {param:12,txtcodmovprof:aORDEN_COMPRA[0].COD_MOV_PRF},

      success : function(listar){
      	$("#divproforma").html(listar);
		//$("#txtnum_proforma").html(aORDEN_COMPRA[0].);
		$("#txtnum_proforma").html('Detalle de Cotización | '+aORDEN_COMPRA[0].NUM_ORDEN_COMPRA)
      }
			...
			$.ajax({
				...
				var json = JSON.parse(data);
					...
					if (json.status == 200){
						...
						$("#txtnum_proforma_1").val(json.NUM_ORDEN_COMPRA);
						$("#txtnum_proforma").html(json.COD_MOV_PRF);
						$("#txtidcliente").val(json.ID_BENEFICIARIO);
						$("#txtfecharegistro").val(json.FECHA_PEDIDO);
						$("#txtestadopago_compra").val(json.ESTADO_ORDEN_COMPRA);
...
function imprimir_ORDEN_COMPRA(id_ORDEN_COMPRA){
	asql="select * from ORDEN_COMPRA_reporte_cz where ID_ORDEN_COMPRA_CZ="+id_ORDEN_COMPRA;
	regresamatrizjsonasync(asql,'aORDEN_COMPRA');
	console.log(aORDEN_COMPRA);
	window.open("ORDEN_COMPRA_reporte.php?local="+aORDEN_COMPRA[0].ID_LOCAL+"&id="+aORDEN_COMPRA[0].COD_MOV_PRF);
}
function reporte_ORDEN_COMPRA(){
	...
	window.open("ORDEN_COMPRA_reporte.php?local="+axidlocal+"&id="+axcodproformacz);
}
...
function Generar_CodPorforma__(){
	...
	var axidlocal= $("#txtidlocal").val();
		...
		asql="insert into ORDEN_COMPRA_CZ(COD_MOV_PRF,ID_LOCAL) VALUES('"+axcodproformacz1+"',"+axidlocal+")";
		console.log(asql);
		regresamatrizjsonasync(asql,"acarga");
		//console.log(acarga);
		
		asql="select ID_ORDEN_COMPRA_CZ from ORDEN_COMPRA_CZ WHERE COD_MOV_PRF='"+axcodproformacz1+"'";
		regresamatrizjsonasync(asql,"acodORDEN_COMPRA");
		console.log(acodORDEN_COMPRA);
		num_ORDEN_COMPRA=("00000"+acodORDEN_COMPRA[0].ID_ORDEN_COMPRA_CZ+"-2024").right(12);
		$('#txtnum_proforma_1').val(num_ORDEN_COMPRA);

...
case '1': // LISTAR ALMACENES MODULO ALMACENES.PHP
...
$axid_empresa = $_POST['txtid_empresa']; 	
	...
	if($axbuscaregistro==""){
		...
		$SQLBuscar = "SELECT * FROM LOCALES WHERE ID_EMPRESA = '$axid_empresa'";
		
	}else{

		$SQLBuscar ="SELECT * FROM LOCALES WHERE ID_EMPRESA = '$axid_empresa' AND RAZON_SOCIAL like '%".$axbuscaregistro."%' ";
	...
	if ($RSBuscar){
 	...
 	echo "
 		<tr> 		
 			<td >".$row["RUC_EMPRESA"].'<br>'.$row["RAZON_SOCIAL"]."</td> 
 			<td ><a href='#' class='btn btn-outline-info btn-sm' id='btn_editar_almacen' name='btn_editar_almacen' data-idlocal='$axid_local' data-bs-toggle='modal' data-bs-target='#exampleModal'><i class='fa-solid fa-pen-to-square'></i> Editar</a></td> 
 		</tr>
 	";
...
$axid_empresa = $_POST['txtid_empresa']; 	
	...
	if($axbuscaregistro==""){
		...
		//$SQLBuscar = "SELECT  TOP 30  ID_PRODUCTO,COD_PRODUCTO,NOM_CATEGORIA,NOM_PRODUCTO,TIPO,PRESENTACION,PROCEDENCIA,ESTADO,CANT_CAJA,COSTO_PRODUCTO,PESO_PRODUCTO FROM PRODUCTOS_LISTADO WHERE ID_EMPRESA = '$axid_empresa'";
		  $SQLBuscar = "SELECT  TOP 30  * FROM PRODUCTOS_LISTADO";
		...
		$SQLBuscar ="SELECT  TOP 30 * WHERE NOM_PRODUCTO+NOM_CATEGORIA+COD_PRODUCTO like '%".$axbuscaregistro."%' ";

	}
	echo '<script language="javascript">console.log("'.$SQLBuscar.'");</script>';
	//echo $SQLBuscar;
	...
	echo "
		...
		<tr>
			...
			<th class='ocultar' style='text-align: left;'>Categoría</th>
			<th style='text-align: left;'>Descripción</th>			
			<th style='text-align: center;'>Presentación</th>
			<th class='ocultar' style='text-align: right;'>Costo</th>						
			<th class='ocultar' style='text-align: right;'>Prc.Venta</th>						
	...
	if ($RSBuscar){
 	...
 	while ($fila=odbc_fetch_array($RSBuscar)){ 
		...
		$costo = number_format($fila['PRECIO_COMPRA_CIGV'],2,".",",");
		$estado = $fila['ESTADO'];
		//$cant_caja = $fila['CANT_CAJA'];
		$axpeso = $fila['PESO_PRODUCTO'];
		$axprecio = number_format($fila['PRECIO_VENTA_CIGV'],2,".",",");
 	...
 	echo "
 		<tr> 		
 			...
 			<td class='ocultar' style='text-align: left;'>".$nom_categoria."</td>
 			<td class='ocultar' style='text-align: left;'>".utf8_encode($nom_producto)."</td>
 			<td style='text-align: center;'>".$presentacion."</td> 
 			<td class='ocultar' style='text-align: right;'>".$costo."</td> 
 			<td class='ocultar' style='text-align: right;'>".$axprecio."</td> 
 			<td style='text-align: right;'><b class='text-danger'>".$axpeso."</b></td>
...
$axid_producto= $_POST['txtid_producto'];
	...
	$sql6 = "SELECT * FROM PRODUCTOS WHERE ID_PRODUCTO = '$axid_producto'";
...
case '8': //grabar productos
	...
	$axestado = $_POST['txtestado_producto'];
	...
	$axprs_minimo = $_POST['txtprecio_minimo'];
	$axprs_menor = intval($_POST['txtprs_menor']);
	$axprs_mayor = intval($_POST['txtprs_mayor']);
	$axprs_premium = intval($_POST['txtprs_premium']);
	...
	$txtcosto_producto_sin = $_POST['txtcosto_producto_sin'];
	$txtigvcosto = $_POST['txtigvcosto'];

	$txtprecio_sin = $_POST['txtprecio_sin'];
	$txtigv_prc_venta = $_POST['txtigv_prc_venta'];
	$txtprecio_venta = $_POST['txtprecio_venta'];
	$txtstock_minimo = $_POST['txtstock_minimo'];
...
//echo "$Insertar";
		...
		$Insertar ="UPDATE PRODUCTOS SET ID_CATEGORIA='$axid_categoria',COD_PRODUCTO='$axcod_producto',ESTADO_PRODUCTO='$axestado',NOM_PRODUCTO='$axnom_producto',PRESENTACION='$axpresentacion',PESO_PRODUCTO='$axpeso_producto',COSTO_PRODUCTO_SIN='$txtcosto_producto_sin',IGV_COSTO='$txtigvcosto',COSTO_PRODUCTO='$axcosto_producto',PRECIO_VENTA_SIN='$txtprecio_sin',IGV_PRC_VENTA='$txtigv_prc_venta',PRECIO_VENTA='$txtprecio_venta',ID_AFECTACION='$axid_afectacion',STOCK_MINIMO='$txtstock_minimo',PRS_MINIMO='$axprs_minimo' WHERE ID_PRODUCTO='$axid_producto'";
...
$SQLEliminar ="DELETE FROM PRODUCTOS WHERE ID_PRODUCTO='$axid_producto'";
...
if($RSEliminar){
	$respuesta ='0'.$SQLEliminar;
	echo $respuesta;
}else{
	$respuesta ='1'.$SQLEliminar;
	echo $respuesta;
}
...
if($RSGrabar){
	...
	if($axparametros==1){
		...
		$SQLActualizar = "UPDATE USUARIO SET CORRELATIVO_CLIENTES='$axcod_interno_actualizar' WHERE ID_USUARIO='$axid_usuario'";
		$RSActualizar = odbc_exec($con,$SQLActualizar);	
	}
...
$axidempresa = $_POST['txtidempresa']; 
	...
	if($axbuscaregistro==""){
		...
		$sql6 ="SELECT * FROM USUARIO WHERE ID_EMPRESA='$axidempresa'";

	}else{

		$sql6 ="SELECT * FROM USUARIO WHERE ID_EMPRESA='$axidempresa' AND USUARIO+NOM_USUARIO like '%".$axbuscaregistro."%' ";
	...
	if ($result6){
 	...
 	echo "
 		<tr>
 			...
 			<td >".$row["CONDICION"]."</td> 
 			<td ><a href='#' class='btn btn-outline-info btn-sm' id='bteditarusuario' name='bteditarusuario' data-iduser='$iduser' data-bs-toggle='modal' data-bs-target='#exampleModal'>Editar</a></td> 
...
if($axcargo=='VENDEDOR'){
	...
	if($axparametros==1){
		...
		$Insertar = "INSERT INTO USUARIO (ID_EMPRESA,COD_USUARIO,USUARIO,NOM_USUARIO,CLAVE,TIPO_USUARIO,F_REGISTRO,CONDICION,CORRELATIVO_VENDEDOR,NUM_LICENCIA,PATERNO,MATERNO,NOMBRES,N_SERIE_VENDEDOR,FILTRO_VENTAS) VALUES ('$axidempresa','$axcoduser','$axuser','$axnomusuario','$axclave','$axcargo','$axfecharegistro','$axcondicion','$axcorrelativo_pedidos','$axnum_licencia','$axpaterno','$axmaterno','$axnombres','$axletra_serie','$axfiltro_ventas')";
//echo "$Insertar";
...
case '70':
	...
	$sql ="DELETE FROM USUARIO_LOCALES WHERE ID_ASIGNACION='$idasignetapa' AND ID_USUARIO ='$axidusuario'";
    $result6=odbc_exec($con,$sql); 
...
case '107':
 ...
 if(isset($_POST["txtdistrito_buscar"])){
	...
	$output ="";
	$idprov ="";
	$sqlemisor = "SELECT TOP 30 * FROM TB_UBIGEOS_LISTA WHERE UBIGEO_PERU LIKE  '%".$axbuscar_dato."%' ORDER BY UBIGEO_PERU ASC";
	//echo $sqlemisor;
...
if($axpermiso_abrir==1){
	$sqletapas = "SELECT * FROM USUARIO  WHERE ID_USUARIO='$axiduser' AND FILTRO_VENTAS='VENDEDOR' ORDER BY NOM_USUARIO asc" ;
}elseif($axpermiso_abrir==0){
	$sqletapas = "SELECT * FROM USUARIO WHERE FILTRO_VENTAS='VENDEDOR' ORDER BY NOM_USUARIO asc" ;
}
